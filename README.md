...
1. **Data Generation & CDC**
   - PostgreSQL contains `rh` (HR data) and `sport_poc` (sport events) databases
   - Logical replication enabled for CDC
   - Debezium captures changes and publishes to Kafka topics

2. **Event Streaming**
   - Redpanda (Kafka) handles events:
...
- **Geospatial**: OSMNX, NetworkX, BAN API
...
- either :
	...
	- Copy `.env.example` to `.env` and replace placeholders with your own
...
First launch might take some time.
